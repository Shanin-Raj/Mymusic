# spotify-library-view Specification
...
- **Playlists active**: Tapping `+` opens a quick "New Playlist" creation dialog directly. This dialog SHALL allow the user to select an optional playlist cover image from a horizontal list of available images fetched from the backend (`/api/images`).
...
To protect against visual occlusion on modern devices, all modular bottom sheets and scrollable option dialogs triggered from the library view MUST apply dynamic bottom padding incorporating the device's system safe area height, ensuring no layout boundaries or actions are hidden behind system navigation bars.

Additionally, the scrollable list container on the Library, Home, and Search screens MUST dynamically adjust its bottom padding depending on whether the mini player is visible or hidden. When the mini player is active (a track is playing), a bottom padding of `80.0` or `100.0` SHALL be applied to prevent the list content from being hidden behind the player. When no track is active, the padding SHALL collapse to a default value (e.g. `16.0`) to avoid unnecessary empty space at the bottom of the screens.
...
#### Scenario: Bottom sheet display padding safety
- **WHEN** any library option bottom sheet or dialog is opened
- **THEN** the layout container applies dynamic bottom padding based on device safe area to prevent navigation bar overlap.
